...
export function emitBun(n: Ninja, cfg: Config, sources: Sources): BunOutput {
  ...
  const embedProbeRustObjects = emitRustArchive(
    ...
    {
      codegenInputs: codegen.rustInputs,
      // The main Windows Rust archive owns the one shim edge. Order the probe
...
function emitEmbedProbeTarget(n: Ninja, cfg: Config, input: EmbedProbeTargetInput): void {
  ...
  const hostBundleSource =
    'globalThis.__nimbusInvoke = async function(request) { if ("__nimbusHostBridgeCallJson" in globalThis) throw new Error("raw Nimbus host bridge must not be guest-visible"); const descriptor = Object.getOwnPropertyDescriptor(globalThis, "__nimbusCreateContext"); if (!descriptor || descriptor.writable || descriptor.configurable || descriptor.enumerable) throw new Error("Nimbus context factory must be immutable and hidden"); JSON.stringify = () => "{\\"abi_version\\":1,\\"operation\\":\\"runtime_extension_call\\",\\"payload\\":null}"; Object.prototype.toJSON = () => ({ abi_version: 1, operation: "runtime_extension_call", payload: null }); const ctx = globalThis.__nimbusCreateContext({ request }); const value = await ctx.db.insert("messages", { body: request.args.body }); return { status: "ok", value }; };';
  ...
  writeIfChanged(
    ...
    [
      ...
      'extern "C" int nimbus_bun_embed_probe_timeout_and_cancel();',
      'extern "C" int nimbus_bun_embed_probe_permission_surface_inventory();',
      ...
      "typedef bool (*NimbusBunEmbedIsCancelledFn)(void*);",
      'extern "C" int nimbus_bun_embed_invoke_program_wrapper_json(const unsigned char*, size_t, const unsigned char*, size_t, const unsigned char*, size_t, unsigned char*, size_t, size_t*, void*, NimbusBunEmbedIsCancelledFn);',
      "typedef int (*NimbusBunEmbedHostCallJsonFn)(void*, const unsigned char*, size_t, unsigned char*, size_t, size_t*);",
      'extern "C" int nimbus_bun_embed_invoke_program_wrapper_json_with_host_bridge(const unsigned char*, size_t, const unsigned char*, size_t, const unsigned char*, size_t, unsigned char*, size_t, size_t*, void*, NimbusBunEmbedHostCallJsonFn, void*, NimbusBunEmbedIsCancelledFn);',
      'extern "C" int nimbus_bun_embed_take_pending_response(unsigned char*, size_t, size_t*);',
      "static std::atomic<int> nimbus_bun_embed_driver_host_calls{0};",
      "static std::atomic<int> nimbus_bun_embed_driver_overflow_host_calls{0};",
      "static std::atomic<size_t> nimbus_bun_embed_driver_pending_host_response{0};",
      "static int nimbus_bun_embed_driver_host_bridge(void*, const unsigned char* input, size_t input_len, unsigned char* output, size_t output_cap, size_t* output_len) {",
      '  static const unsigned char expected[] = "{\\"abi_version\\":1,\\"operation\\":\\"document_insert\\",\\"payload\\":{\\"host_call_session_id\\":\\"session-1\\",\\"table\\":\\"messages\\",\\"fields\\":{\\"body\\":\\"hello\\"}}}";',
      "  if (input_len != sizeof(expected) - 1) return 2;",
      "  for (size_t i = 0; i < sizeof(expected) - 1; ++i) {",
      "    if (input[i] != expected[i]) return 2;",
      "  }",
      "  nimbus_bun_embed_driver_host_calls.fetch_add(1);",
      '  const unsigned char response[] = "{\\"status\\":\\"ok\\",\\"value\\":\\"driver-host-value\\"}";',
      "  const size_t response_len = sizeof(response) - 1;",
      "  *output_len = response_len;",
      "  if (response_len > output_cap) return 1;",
      "  for (size_t i = 0; i < response_len; ++i) output[i] = response[i];",
      "  return 0;",
      "}",
      "static int nimbus_bun_embed_driver_overflow_host_bridge(void*, const unsigned char* input, size_t input_len, unsigned char* output, size_t output_cap, size_t* output_len) {",
      "  if (input == nullptr) {",
      "    if (input_len != 0) return 300;",
      "    const size_t response_len = nimbus_bun_embed_driver_pending_host_response.exchange(0);",
      "    if (response_len == 0) return 320;",
      "    *output_len = response_len;",
      "    if (response_len > output_cap) { nimbus_bun_embed_driver_pending_host_response.store(response_len); return 307; }",
      '    static const unsigned char prefix[] = "{\\"status\\":\\"ok\\",\\"value\\":\\"";',
      '    static const unsigned char suffix[] = "\\"}";',
      "    const size_t prefix_len = sizeof(prefix) - 1;",
      "    const size_t suffix_len = sizeof(suffix) - 1;",
      "    for (size_t i = 0; i < prefix_len; ++i) output[i] = prefix[i];",
      "    for (size_t i = prefix_len; i < response_len - suffix_len; ++i) output[i] = 'x';",
      "    for (size_t i = 0; i < suffix_len; ++i) output[response_len - suffix_len + i] = suffix[i];",
      "    return 0;",
      "  }",
      "  nimbus_bun_embed_driver_overflow_host_calls.fetch_add(1);",
      "  const size_t response_len = output_cap + 1;",
      "  nimbus_bun_embed_driver_pending_host_response.store(response_len);",
      "  *output_len = response_len;",
      "  return 307;",
      "}",
      ...
      "}",
      "static bool nimbus_bun_embed_driver_is_explicitly_cancelled(void* context) {",
      "  return static_cast<NimbusBunEmbedCancellationProof*>(context)->cancelled.load();",
      "}",
      "static int nimbus_bun_embed_driver_exit_code(int status) {",
      '  std::fprintf(stderr, "Nimbus Bun/JSC embed probe failed with status %d\\n", status);',
      ...
      "  if (status != 0) return nimbus_bun_embed_driver_exit_code(status);",
      "  status = nimbus_bun_embed_probe_program_bundle_host_calls();",
      "  if (status != 0) return nimbus_bun_embed_driver_exit_code(status);",
      "  status = nimbus_bun_embed_probe_timeout_and_cancel();",
      "  if (status != 0) return nimbus_bun_embed_driver_exit_code(status);",
      "  status = nimbus_bun_embed_probe_permission_surface_inventory();",
      "  if (status != 0) return nimbus_bun_embed_driver_exit_code(status);",
      "  status = nimbus_bun_embed_probe_memory_behavior();",
      "  if (status != 0) return nimbus_bun_embed_driver_exit_code(status);",
      ...
      "  status = nimbus_bun_embed_invoke_program_wrapper_json(bundle, sizeof(bundle) - 1, nullptr, 0, request, sizeof(request) - 1, output, sizeof(output), &output_len, &normal_cancellation_proof, nimbus_bun_embed_driver_is_explicitly_cancelled);",
      "  if (status != 313) return 249;",
      "  status = nimbus_bun_embed_invoke_program_wrapper_json(bundle, sizeof(bundle) - 1, bundle_sha256, sizeof(bundle_sha256) - 1, request, sizeof(request) - 1, output, sizeof(output), &output_len, &normal_cancellation_proof, nullptr);",
      "  if (status != 300) return 248;",
      "  status = nimbus_bun_embed_invoke_program_wrapper_json(bundle, sizeof(bundle) - 1, bundle_sha256, sizeof(bundle_sha256) - 1, request, sizeof(request) - 1, output, sizeof(output), &output_len, &normal_cancellation_proof, nimbus_bun_embed_driver_is_explicitly_cancelled);",
      "  if (status != 0) return nimbus_bun_embed_driver_exit_code(status);",
      '  const unsigned char invalid_request[] = "null); globalThis.__nimbusRequestInjection = true; //";',
      "  output_len = 0;",
      "  status = nimbus_bun_embed_invoke_program_wrapper_json(bundle, sizeof(bundle) - 1, bundle_sha256, sizeof(bundle_sha256) - 1, invalid_request, sizeof(invalid_request) - 1, output, sizeof(output), &output_len, &normal_cancellation_proof, nimbus_bun_embed_driver_is_explicitly_cancelled);",
      ...
      "  status = nimbus_bun_embed_invoke_program_wrapper_json(bundle, sizeof(bundle) - 1, wrong_sha256, sizeof(wrong_sha256) - 1, request, sizeof(request) - 1, output, sizeof(output), &output_len, &normal_cancellation_proof, nimbus_bun_embed_driver_is_explicitly_cancelled);",
      ...
      `  const unsigned char invalid_request_side_effect_bundle[] = ${JSON.stringify(invalidRequestSideEffectBundleSource)};`,
      `  const unsigned char invalid_request_side_effect_bundle_sha256[] = "${sha256(invalidRequestSideEffectBundleSource)}";`,
      `  const unsigned char host_overflow_bundle[] = ${JSON.stringify(hostOverflowBundleSource)};`,
      `  const unsigned char host_overflow_bundle_sha256[] = "${sha256(hostOverflowBundleSource)}";`,
      "  output_len = 0;",
      "  status = nimbus_bun_embed_invoke_program_wrapper_json_with_host_bridge(invalid_request_side_effect_bundle, sizeof(invalid_request_side_effect_bundle) - 1, invalid_request_side_effect_bundle_sha256, sizeof(invalid_request_side_effect_bundle_sha256) - 1, invalid_request, sizeof(invalid_request) - 1, output, sizeof(output), &output_len, (void*)1, nimbus_bun_embed_driver_host_bridge, &normal_cancellation_proof, nimbus_bun_embed_driver_is_explicitly_cancelled);",
      "  if (status != 301) return 246;",
      "  if (nimbus_bun_embed_driver_host_calls.load() != 0) return 245;",
      "  output_len = 0;",
      ...
      "  for (size_t i = 0; i < sizeof(expected_host_output) - 1; ++i) {",
      "    if (output[i] != expected_host_output[i]) return 250;",
      "  }",
      ...
      "  size_t retry_len = 0;",
      "  status = nimbus_bun_embed_take_pending_response(tiny_output, sizeof(tiny_output), &retry_len);",
      "  if (status != 307 || retry_len != pending_len) return 241;",
      "  output_len = 0;",
      "  status = nimbus_bun_embed_take_pending_response(output, sizeof(output), &output_len);",
      "  if (status != 0 || output_len != pending_len) return 240;",
      "  if (nimbus_bun_embed_driver_host_calls.load() != 2) return 239;",
      "  status = nimbus_bun_embed_take_pending_response(output, sizeof(output), &output_len);",
  ...
  // Bun 1.4.1 uses the same direct Rust archive inputs for the executable,
  // the probe, and the shared embedder.
  const rustLinkInputs = input.rustObjects;
...
function emitEmbedderSharedTarget(n: Ninja, cfg: Config, input: EmbedProbeTargetInput): void {
  ...
  const smokeStamp = emitEmbedderSharedSmokeTest(n, cfg, shared);
  n.phony("bun-embed-shared", [shared]);
  n.phony("check-bun-embed-shared", [smokeStamp]);
}
...
function emitEmbedderSharedSmokeTest(n: Ninja, cfg: Config, shared: string): string {
  ...
  writeIfChanged(
    ...
    [
      ...
      "import sys",
      "",
      'os.environ["NIMBUS_BUN_EMBED_PROBE_TRACE"] = "1"',
      'print(f"loading shared adapter: {sys.argv[1]}", flush=True)',
      "library = ctypes.CDLL(str(pathlib.Path(sys.argv[1]).resolve()))",
      'print("shared adapter loaded", flush=True)',
      "for name in [",
      ...
      "    status = probe()",
      "    if status != 0:",
      '        raise SystemExit(f"{name} returned {status}")',
      '    print(f"passed {name}", flush=True)',
      "",
      "bundle = b'globalThis.__nimbusInvoke = async request => ({status: \"ok\", value: request.args});'",
      'request = b\'{"kind":"query","function_name":"messages:sharedProof","args":{"loaded":true}}\'',
      "expected_sha256 = hashlib.sha256(bundle).hexdigest().encode()",
      "output = ctypes.create_string_buffer(1024)",
      "output_len = ctypes.c_size_t()",
      "invoke = library.nimbus_bun_embed_invoke_program_wrapper_json",
      "byte_pointer = ctypes.POINTER(ctypes.c_ubyte)",
      "is_cancelled_callback = ctypes.CFUNCTYPE(ctypes.c_bool, ctypes.c_void_p)",
      "never_cancelled = is_cancelled_callback(lambda _context: False)",
      "invoke.argtypes = [",
      "    byte_pointer, ctypes.c_size_t, byte_pointer, ctypes.c_size_t,",
      "    byte_pointer, ctypes.c_size_t, byte_pointer, ctypes.c_size_t,",
      "    ctypes.POINTER(ctypes.c_size_t), ctypes.c_void_p, is_cancelled_callback,",
      "]",
      "invoke.restype = ctypes.c_int",
      "take_pending = library.nimbus_bun_embed_take_pending_response",
      "take_pending.argtypes = [byte_pointer, ctypes.c_size_t, ctypes.POINTER(ctypes.c_size_t)]",
      "take_pending.restype = ctypes.c_int",
      'print("running nimbus_bun_embed_invoke_program_wrapper_json", flush=True)',
      "status = invoke(",
      "    ctypes.cast(ctypes.c_char_p(bundle), byte_pointer), len(bundle),",
      "    ctypes.cast(ctypes.c_char_p(expected_sha256), byte_pointer), len(expected_sha256),",
      "    ctypes.cast(ctypes.c_char_p(request), byte_pointer), len(request),",
      "    ctypes.cast(output, byte_pointer), len(output), ctypes.byref(output_len), ctypes.c_void_p(1), never_cancelled,",
      ")",
      "if status != 0:",
      '    raise SystemExit(f"nimbus_bun_embed_invoke_program_wrapper_json returned {status}")',
      'print("passed nimbus_bun_embed_invoke_program_wrapper_json", flush=True)',
      "response = json.loads(output.raw[:output_len.value])",
      'if response != {"status": "ok", "value": {"loaded": True}}:',
      '    raise SystemExit(f"unexpected shared adapter response: {response!r}")',
      "tiny_output = ctypes.create_string_buffer(1)",
      "pending_len = ctypes.c_size_t()",
      "status = invoke(",
      "    ctypes.cast(ctypes.c_char_p(bundle), byte_pointer), len(bundle),",
      "    ctypes.cast(ctypes.c_char_p(expected_sha256), byte_pointer), len(expected_sha256),",
      "    ctypes.cast(ctypes.c_char_p(request), byte_pointer), len(request),",
      "    ctypes.cast(tiny_output, byte_pointer), len(tiny_output), ctypes.byref(pending_len), ctypes.c_void_p(1), never_cancelled,",
      ")",
      "if status != 307 or pending_len.value <= len(tiny_output):",
      '    raise SystemExit(f"oversized response was not retained: status={status}, length={pending_len.value}")',
      "recovered = ctypes.create_string_buffer(pending_len.value)",
      "recovered_len = ctypes.c_size_t()",
      "status = take_pending(ctypes.cast(recovered, byte_pointer), len(recovered), ctypes.byref(recovered_len))",
      "if status != 0 or recovered_len.value != pending_len.value:",
      '    raise SystemExit(f"pending response retrieval failed: status={status}, length={recovered_len.value}")',
      "if json.loads(recovered.raw[:recovered_len.value]) != response:",
      '    raise SystemExit("pending response retrieval changed the completed response")',
      'print("passed nimbus_bun_embed_take_pending_response", flush=True)',
      "",
    ].join("\n"),
  );

  const q = (path: string) => quote(path, cfg.windows);
  n.rule("embed_shared_smoke_test", {
    command: [
      cfg.jsRuntime,
      q(streamPath),
      "check-bun-embed-shared",
      "--console",
    ...
    description: "bun-embed-shared",
    pool: "console",
  });
  n.build({ outputs: [stamp], rule: "embed_shared_smoke_test", inputs: [shared, loader] });
  return stamp;
}
...
function sharedEmbedderExportList(cfg: Config): string {
  if (cfg.darwin) {
    return [
      ...
      "_nimbus_bun_embed_invoke_program_wrapper_json_with_host_bridge",
      "_nimbus_bun_embed_take_pending_response",
      "",
    ].join("\n");
  }
